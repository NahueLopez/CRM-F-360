...
const ConfirmModal: React.FC<ConfirmModalProps> = ({
  ...
  return (
    <div
      className="fixed inset-0 z-100 flex items-center justify-center p-4 h-dvh w-full"
      role="dialog"
      aria-modal="true"
    ...
    >
      {/* Backdrop */}
      <div
        className="fixed inset-0 bg-black/60 backdrop-blur-sm confirm-modal-backdrop"
        onClick={onCancel}
        aria-hidden="true"
      />
      {/* Modal */}
      <div className="relative bg-slate-800 border border-slate-700/60 rounded-2xl shadow-2xl shadow-black/40 w-full max-w-sm p-6 confirm-modal-content">
        <div className="flex flex-col items-center text-center">
          <div
            className={`w-14 h-14 rounded-2xl ${cfg.iconBg} border flex items-center justify-center text-2xl mb-4`}
          >
            {cfg.icon}
          </div>
