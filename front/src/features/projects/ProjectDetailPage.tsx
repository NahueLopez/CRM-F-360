    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div>
          <Link
            to="/projects"
            className="text-xs text-slate-500 hover:text-slate-300 transition mb-1 inline-block"
          >
            ← Proyectos
          </Link>
          ...
          <div className="flex items-center gap-2 mt-1">
            <span
              className={`w-2 h-2 rounded-full ${STATUS_COLORS[project.status] ?? "bg-slate-500"}`}
            />
          ...
          {estimatedHours > 0 ? (
            <div className="mt-2">
              <div className="flex items-center gap-2">
                ...
                <span
                  className={`text-sm font-bold ${burnPercent > 100 ? "text-red-400" : "text-emerald-400"}`}
                >
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-4 text-xs text-slate-400">
                    ...
                    <span className="tabular-nums text-indigo-400 font-medium">
        ...
        {recentActivities.length > 0 ? (
          <div className="space-y-2">
            {recentActivities.map((a) => (
              <div
                key={a.id}
                className="flex items-start gap-3 py-2 px-3 rounded-lg hover:bg-slate-800/50 transition"
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-slate-500 mt-0.5">
                    {a.userName} · {new Date(a.createdAt).toLocaleDateString("es-AR")}{" "}
      ...
      {(project.startDate || project.endDateEstimated) && (
        <div className="flex items-center gap-6 text-xs text-slate-500 px-1">
          ...
          {project.endDateEstimated && (
            <span>
              🏁 Cierre estimado: {new Date(project.endDateEstimated).toLocaleDateString("es-AR")}
            </span>
          )}
        </div>
      )}
