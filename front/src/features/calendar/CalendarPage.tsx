...
const CalendarPage: React.FC = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        setLoading(true);
        const [t, p] = await Promise.all([taskService.getAll(), projectService.getAll()]);
        setTasks(t);
        setProjects(p);
      } catch (err) {
  ...
  const handleCancelReservation = async (id: number) => {
    ...
    try {
      ...
      addToast("error", "Error al cancelar");
    }
  };
  ...
  const handleQuickAddTask = async (data: {
    ...
    try {
      ...
      addToast("error", "Error al crear tarea");
    }
  };
            ...
            {(() => {
              ...
              return (
                <div>
                  ...
                  {showAddTask && (
                    <QuickTaskForm projects={projects} onSubmit={handleQuickAddTask} />
                  )}
                  ...
                  {dayTasks.length > 0 && (
                    <div className="space-y-1 mt-2">
                      {dayTasks.map((t) => (
                        ...
                        >
                          <span
                            className={`w-2 h-2 rounded-full shrink-0 ${priorityColors[t.priority] ?? priorityColors.Medium}`}
                          />
                          <div className="flex-1 min-w-0">
                            <p className="text-[11px] font-medium text-slate-200 truncate">
                              {t.title}
