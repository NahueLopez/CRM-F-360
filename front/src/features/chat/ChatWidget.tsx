...
const ChatWidget: React.FC = () => {
          ...
          .map((c) =>
            c.id === msg.conversationId
              ? {
                  ...
                  unreadCount: msg.senderId !== myId ? c.unreadCount + 1 : c.unreadCount,
                }
              : c,
          )
          .sort(
          ...
          c.id === convId
            ? {
                ...
                participants: c.participants.map((p) =>
                  p.userId === _userId ? { ...p, lastReadAt: readAt } : p,
                ),
              }
            : c,
        ),
      ...
      setReadReceipts((prev) => {
        ...
        for (const c of convs) {
          ...
          if (readTimes.length === others.length && readTimes.length > 0) {
            const minRead = readTimes.reduce((a, b) =>
              new Date(norm(a)) < new Date(norm(b)) ? a : b,
            );
  ...
  const openConversation = async (conv: Conversation) => {
    ...
    try {
      ...
      console.error("[Chat] openConversation error:", err);
    }
    setLoading(false);
  ...
  const handleSend = async () => {
    ...
    const replyContent = replyTo
      ? `> ${replyTo.senderName.split(" ")[0]}: ${replyTo.content.slice(0, 80)}\n\n${text}`
      : text;
    setDraft("");
    setReplyTo(null);
  ...
  const msgTime = (iso: string) => {
    ...
    if (diff < 7 * 86400000)
      return d.toLocaleDateString("es-AR", { weekday: "short" }) + " " + hhmm;
  ...
  const typingLabel =
    activeConv && typingUsers.has(activeConv.id)
      ? (users.find((u) => u.id === typingUsers.get(activeConv.id))?.fullName?.split(" ")[0] ??
        "Alguien")
      : null;
  ...
  const filteredConversations = useMemo(() => {
    ...
    return conversations.filter(
      (c) => c.name?.toLowerCase().includes(q) || c.lastMessageContent?.toLowerCase().includes(q),
    );
  ...
  const ReceiptIcon = ({ status }: { status: string }) => {
    ...
    if (status === "failed")
      return (
        <span className="text-red-400 text-[9px]" title="Error al enviar">
          ⚠️
        </span>
      );
  ...
  return (
    <div
      className={`fixed bottom-5 right-5 z-50 ${widgetW} ${widgetH} flex flex-col overflow-hidden
    ...
    >
        ...
        {screen === "list" && (
          <>
            ...
            <div className="flex items-center gap-0.5">
              ...
              <button
                onClick={handleClose}
                className="p-2 rounded-lg text-slate-400 hover:text-white hover:bg-red-500/20 transition-all"
              >
                <svg
                  width="14"
                  height="14"
        ...
        {screen === "chat" && activeConv && (
          <>
            <button
              ...
              className="p-1.5 rounded-lg text-slate-400 hover:text-white hover:bg-slate-700/50 transition-all mr-2"
            >
              <svg
                width="16"
                height="16"
            ...
            <div className="flex-1 min-w-0">
              ...
              {typingLabel ? (
                ...
                <p className="text-[10px] text-slate-500 leading-tight mt-0.5">
                  {activeConv.participants.length} miembros
                </p>
              ) : (
                <p className="text-[10px] leading-tight mt-0.5">
                  {isUserOnline(
                    activeConv.participants.find((p) => p.userId !== myId)?.userId ?? 0,
                  ) ? (
        ...
        {(screen === "newDm" || screen === "newGroup") && (
          <>
            ...
            <h3 className="text-sm font-semibold flex-1">
              {screen === "newDm" ? "Nuevo mensaje" : "Nuevo grupo"}
            </h3>
            <button
              onClick={handleClose}
              className="p-2 rounded-lg text-slate-400 hover:text-white hover:bg-red-500/20 transition-all"
            ...
            {filteredConversations.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-center px-6">
                ...
                <p className="text-[11px] text-slate-600 mt-1">
                  {searchQuery ? "Intentá con otro término" : "Tocá ✏️ para iniciar un chat"}
                </p>
              </div>
            ) : (
                ...
                return (
                  ...
                  >
                    <div className="relative shrink-0">
                      <div
                        className={`w-11 h-11 rounded-full flex items-center justify-center text-xs font-bold transition-transform group-hover:scale-105 ${conv.isGroup ? "bg-gradient-to-br from-violet-500/25 to-purple-600/25 text-violet-400" : "bg-gradient-to-br from-indigo-500/25 to-blue-600/25 text-indigo-400"}`}
                      >
            ...
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center h-full text-center">
                ...
                <p className="text-[10px] text-slate-600 mt-1">
                  Iniciá la conversación con {activeConv.name}
                </p>
              </div>
            ) : (
              messagesWithDates.map((item, idx) => {
                if (item.type === "date") {
                  return (
                    <div key={`date-${idx}`} className="flex items-center gap-3 my-4">
                      ...
                      <span className="text-[10px] text-slate-500 font-medium px-2 py-0.5 rounded-full bg-slate-800/60 shrink-0">
                        {item.date}
                      </span>
                      <div className="flex-1 h-px bg-slate-700/40" />
                    </div>
                ...
                const showAvatar =
                  !isMe &&
                  (prevMsg?.type === "date" ||
                    (prevMsg?.type === "msg" && prevMsg.msg?.senderId !== msg.senderId));
                ...
                return (
                  ...
                  >
                    ...
                    <div className="relative max-w-[78%]">
                      ...
                      <div
                        className={`px-3 py-2 rounded-2xl text-[13px] leading-relaxed transition-all ${
                          msg._optimistic && !msg._failed ? "opacity-70" : ""
                        } ${msg._failed ? "opacity-60 ring-1 ring-red-500/50" : ""} ${
                          isMe
                      ...
                      >
                        ...
                        {isReplyMsg &&
                          (() => {
                            ...
                            return (
                              <>
                                <div
                                  className={`text-[11px] mb-1.5 pl-2 border-l-2 ${isMe ? "border-indigo-300/40 text-indigo-200/70" : "border-slate-600 text-slate-400"} py-0.5`}
                                >
                                  {quoteLine}
                                </div>
                        ...
                        <div
                          className={`text-[9px] mt-1 ${isMe ? "text-indigo-200/50" : "text-slate-600"} text-right flex items-center justify-end gap-1`}
                        >
                          {msgTime(msg.sentAt)}
                      ...
                      {msg._failed && (
                        <button
                          onClick={handleSend}
                          className="text-[10px] text-red-400 hover:text-red-300 mt-0.5 ml-1 transition-colors"
                        >
                          Reintentar envío
                        </button>
                      )}
      ...
      {screen === "newDm" && (
        <div className="flex-1 overflow-y-auto">
          <p className="px-4 pt-3 pb-2 text-[10px] text-slate-500 uppercase tracking-wider font-medium">
      ...
      {screen === "newGroup" && (
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
            ...
            <div className="flex flex-wrap gap-1.5">
              {groupMembers.map((uid) => {
                ...
                return (
                  <span
                    key={uid}
                    className="px-2.5 py-1 rounded-full bg-indigo-500/15 text-indigo-400 text-[11px] font-medium flex items-center gap-1.5 border border-indigo-500/20"
                  >
                    {u?.fullName?.split(" ")[0]}
                    <button
                      onClick={() => setGroupMembers((prev) => prev.filter((id) => id !== uid))}
          ...
          <div className="space-y-0.5 max-h-48 overflow-y-auto">
            {users
              .filter((u) => !groupMembers.includes(u.id))
              .map((u) => (
                <button
                  key={u.id}
