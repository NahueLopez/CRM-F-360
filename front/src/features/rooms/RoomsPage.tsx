    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Gestioná las salas de reuniones. Para reservar, usá el{" "}
            <span className="text-indigo-400">Calendario</span>.
          </p>
        </div>
        {isManager && (
          <button
      ...
      {rooms.length === 0 ? (
        <div className="text-center py-20">
          <div className="w-16 h-16 rounded-2xl bg-slate-800/60 border border-slate-700/30 flex items-center justify-center text-3xl mx-auto mb-4 opacity-40">
            🏢
          </div>
...
const RoomCard = ({ room, onDelete }: { room: Room; onDelete?: () => void }) => (
  <div
  ...
  >
    ...
    <div className="flex items-start justify-between mb-3">
      <div>
        ...
        {room.location && <p className="text-[10px] text-slate-500 mt-0.5">📍 {room.location}</p>}
      </div>
      <span
  ...
  <div className="space-y-6 animate-pulse">
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
      {Array.from({ length: 4 }).map((_, i) => (
        <div
          key={i}
          className="rounded-2xl border border-slate-700/30 bg-slate-800/30 p-5 space-y-3"
        >
