...
const DealFormModal: React.FC<Props> = ({
  form,
  companies,
  isEdit = false,
  onChange,
      ...
      <div className="bg-slate-800 border border-slate-700 rounded-xl w-full max-w-md p-6 space-y-4 shadow-2xl">
        ...
        <div className="flex justify-between items-center mt-4">
          {isEdit && onDelete ? (
            <button
              onClick={onDelete}
              className="px-3 py-1.5 rounded-lg text-xs font-medium text-red-400/70 hover:text-red-400 hover:bg-red-500/10 transition-all"
            >
          ...
          <div className="flex justify-end gap-2">
            <button
              onClick={onClose}
              className="px-4 py-2 rounded-lg border border-slate-600 hover:bg-slate-700/50 text-sm transition text-slate-200"
            >
              Cancelar
            </button>
            <button
              onClick={onSubmit}
              className="px-5 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white font-medium text-sm transition shadow-sm"
            >
