        ...
        <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-4 flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-red-300">
              {overdue.length} recordatorio{overdue.length > 1 ? "s" : ""} vencido
              {overdue.length > 1 ? "s" : ""}
            </p>
      ...
      <div className="space-y-2">
        {filtered.length === 0 ? (
          <div className="text-center py-12 text-slate-500">
            ...
            <p className="text-sm">
              No hay recordatorios {filter === "pending" ? "pendientes" : ""}
            </p>
          </div>
        ) : (
          filtered.map((r) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p
                  className={`text-sm font-medium ${r.isCompleted ? "line-through text-slate-500" : ""}`}
                >
                  {r.title}
                </p>
                {r.description && (
                ...
                <div className="flex items-center gap-3 mt-1">
                  <span
                    className={`text-[10px] ${isOverdue(r) ? "text-red-400" : isDueToday(r) ? "text-amber-400" : "text-slate-500"}`}
                  >
                    📅{" "}
                  ...
                  {r.contactName && (
                    <span className="text-[10px] text-slate-500">👤 {r.contactName}</span>
                  )}
                  {r.companyName && (
