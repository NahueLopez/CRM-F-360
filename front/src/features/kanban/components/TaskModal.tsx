...
import type { TaskComment } from "../../tasks/commentTypes";
import { taskCommentService } from "../../tasks/taskCommentService";
...
const TaskModal: React.FC<Props> = ({ initial, users, onSubmit, onDelete, onClose }) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-slate-800 border border-slate-700 rounded-xl w-full max-w-2xl shadow-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between p-6 pb-0">
          <h3 className="text-lg font-semibold">{initial ? "Editar tarea" : "Nueva tarea"}</h3>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="overflow-y-auto flex-1 p-6 pt-4 space-y-4">
          <form onSubmit={handleSubmit} id="task-form" className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <label className="block text-xs text-slate-400 mb-1">Asignado</label>
                <select
                  value={assigneeId ?? ""}
                  onChange={(e) =>
