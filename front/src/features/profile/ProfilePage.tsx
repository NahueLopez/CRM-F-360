...
const ProfilePage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    authStore.refreshSession().then(() => {
      if (mounted) setUser(authStore.user);
    });
    return () => {
      mounted = false;
    };
    ...
    <div className="w-full max-w-5xl xl:max-w-6xl mx-auto space-y-6">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">
            Administrá tu información personal y de seguridad.
          </p>
        </div>
      </div>
      ...
      <div className="bg-slate-800/30 border border-slate-700/50 rounded-2xl p-6 sm:p-8 shadow-sm">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">
              Mantené tu cuenta segura actualizando tu contraseña periódicamente.
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleChangePassword} className="space-y-5 max-w-xl">
          ...
          {message && (
            <div
              className={`text-xs px-4 py-3 rounded-xl flex items-center gap-2 ${
